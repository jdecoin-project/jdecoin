...
#if defined(NDEBUG)
# error "Jdecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf99d9ea52b4de87ee0cf65829bcc44990f2951e3cf0fb0b2c15298a053d3a046");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jdecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jdecoin Signed Message:\n";
...
{
    // Jdecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight < 12) //premine bounties
	{
	...
	// Subsidy is cut in half every 50,000 blocks, which will occur approximately every 3 years
    nSubsidy >>= (nHeight / 50000); 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Jdecoin: 1 day
static const int64 nTargetSpacing = 20; // Jdecoin: 20 sec
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jdecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jdecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4efe17f04627c9d58ae03193b52d7c6bf3c46cf5e21f9cd04f7399d3b31c2f67");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Jodie and Jade are my life";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 200 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0437601bfa837e23de9cbc050664abfd7d97d14ffbad467de12b8fa3e9368ed8ee306566f1ccca9af47783d741e18f98626e52d9a06167dfeb85cf9542cccf84a3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525198017;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087355497;

        if (fTestNet)
        {
            block.nTime    = 1525198002;
            block.nNonce   = 391421563;
    ...
    if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5ed7f42d5d38f9bb49033870a65c96ffc4f79315e9c52b47a743ab50dd5e2cb8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb3, 0xd1 }; // Jdecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JdecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JdecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JdecoinMiner(CWallet *pwallet)
{
    printf("JdecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jdecoin-miner");
    ...
    try { loop {
        ...
        printf("Running JdecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JdecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JdecoinMiner, pwallet));
}
